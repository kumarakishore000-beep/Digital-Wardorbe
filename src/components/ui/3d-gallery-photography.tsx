	...
	return new THREE.ShaderMaterial({
		...
		fragmentShader: `
			...
			void main() {
				...
				if (blurAmount > 0.0) {
					vec2 texelSize = vec2(1.0 / 512.0) * blurAmount;
					vec4 blurred = vec4(0.0);
					blurred += texture2D(map, uv + vec2(-texelSize.x, -texelSize.y)) * 0.0625;
					blurred += texture2D(map, uv + vec2(0.0, -texelSize.y)) * 0.125;
					blurred += texture2D(map, uv + vec2(texelSize.x, -texelSize.y)) * 0.0625;
					blurred += texture2D(map, uv + vec2(-texelSize.x, 0.0)) * 0.125;
					blurred += texture2D(map, uv) * 0.25;
					blurred += texture2D(map, uv + vec2(texelSize.x, 0.0)) * 0.125;
					blurred += texture2D(map, uv + vec2(-texelSize.x, texelSize.y)) * 0.0625;
					blurred += texture2D(map, uv + vec2(0.0, texelSize.y)) * 0.125;
					blurred += texture2D(map, uv + vec2(texelSize.x, texelSize.y)) * 0.0625;
	...
	return (
		<div className="flex gap-4 overflow-x-auto p-4 max-w-full">
			{normalized.map((img, index) => (
				<div
					key={index}
					className="flex-none w-48 h-64 relative rounded-lg overflow-hidden"
				>
					<img
						src={img.src}
...
function ImagePlane({
	...
	if (material && material.uniforms && material.uniforms.map && texture) {
...
function GalleryScene({
	...
	const spatialPositions = useMemo(() => {
		...
		for (let i = 0; i < visibleCount; i++) {
			...
			const x =
				...
				3;
			const y =
				(Math.cos(verticalAngle) * verticalRadius * maxVerticalOffset) / 3;
	...
	// Use memoized planes data array to avoid ref access during render
	...
	useFrame((_, delta) => {
		if (autoPlay) {
			...
			setScrollVelocity((prev) => prev + (baseSpeed - prev) * 0.1);
		} else {
		...
		const zDelta = scrollVelocity * delta * 10;
		const totalRange = depthRange;

		planesData.forEach((plane, i) => {
			let newZ = plane.z + zDelta;

			let wrapsForward = 0;
			let wrapsBackward = 0;
			const imageAdvance = visibleCount;
...
export default function InfiniteGallery({
	...
	return (
		<GalleryErrorBoundary fallback={fallbackUI}>
			<div className={className} style={style}>
				<Canvas
